...
class Actions_CmsController extends Rastor_Controller_Cms_ActionTable {

    protected $_modelClassName = 'Actions_Model_Action';
    ...
    public function addAction() {
        Core_View_Helper_CmsTitle::getTitle('Новая акция');
        $form = new Actions_Form_CmsAction();
		...
		$formData['date'] =  strtotime($formData['date']);
                if ($this->getModel()->getDbTable()->insert($formData + $preview)) {
                    ...
                    $this->_redirect(Rastor_Url::get('admin', array('module' => 'actions', 'controller' => 'cms', 'action' => 'showlist')));
                }
            }
        }
    }
