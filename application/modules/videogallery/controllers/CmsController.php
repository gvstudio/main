<?php

class VideoGallery_CmsController extends Rastor_Controller_Cms_ActionTable {

    protected $_modelClassName = 'VideoGallery_Model_VideoGallery';
    protected $_tableParams = array('id', 'name', 'enable');
    protected $_tableColumns = array('id', 'Название', 'Активность');
    protected $_tableColumnsWidth = array(40, 0, 100);

    public function addAction() {
        Core_View_Helper_CmsTitle::getTitle('Новый видеоматериал');
        $form = new  VideoGallery_Form_CmsVideoGallery();
        //$form->removeElement('uri');

        $picture = new Core_Model_Pictures(array(
                    'type' => 'picture',
                    'previewWidth' => 137,
                    'previewHeight' => 137,
                    'fixedThumbSize' => true
                ));

        $this->view->json = $picture->getJSONObject();

        if (!$this->getRequest()->isPost()) {
            $this->view->form = $form;
        } else {
            if (!$form->isValid($_POST)) {
                $this->view->form = $form;
            } else {
                $formData = $form->getValues();

                $preview = Zend_Json::decode($formData['preview']);
                unset($formData['preview']);
		$formData['date'] =  strtotime($formData['date']);
                if ($this->getModel()->getDbTable()->insert($formData + $preview)) {
                    $messager = new Rastor_Controller_Cms_Messager();
                    $messager->setAction('successfully_added');
                    $this->_redirect(Rastor_Url::get('admin', array('module' => 'videogallery', 'controller' => 'cms', 'action' => 'showlist')));
                }
            }
        }
    }

    public function editAction() {
        Core_View_Helper_CmsTitle::getTitle('Редактирование видеоматериала');
        $id = $this->_getParam('id', 0);

        if ($data = $this->getModel()->getDbTable()->getRecord($id)) {
            $form = new  VideoGallery_Form_CmsVideoGallery();
	    $data->date=date('d.m.Y',$data->date); 
            $form->setValues($data);
            $form->getElement('submit')->setLabel('Изменить');

            $picture = new Core_Model_Pictures(array(
                        'type' => 'picture',
                        'previewWidth' => 137,
                        'previewHeight' => 137,
                        'fixedThumbSize' => true
                    ));

            $picture->setData(Zend_Json::encode(array(
                        'picture' => $data->picture,
                        'thumb' => $data->thumb
                    )));

            $this->view->json = $picture->getJSONObject();
			//Zend_Debug::dump( $this->view->json); die;

            if (!$this->getRequest()->isPost()) {
                $this->view->form = $form;
            } else {
                if (!$form->isValid($_POST)) {
                    $this->view->form = $form;
                } else {
                    $formData = $form->getValues();
                    
                    $preview = Zend_Json::decode($formData['preview']);
		    $formData['date'] =  strtotime($formData['date']);
                    unset($formData['preview']);
			//Zend_Debug::dump($formData + $preview);die;
			if(empty($preview)){
				$preview['picture']="";
				$preview['thumb']="";
			}
			       if ($this->getModel()->getDbTable()->update($formData + $preview, $id)) {
                        $messager = new Rastor_Controller_Cms_Messager();
                        $messager->setAction('successfully_changed');
                        Rastor_Callback::callback();
                    } else {
                        $messager = new Rastor_Controller_Cms_Messager();
                        $messager->setAction('not_changed');
                    }
                    $this->_redirect(Rastor_Url::get('admin', array('module' => 'videogallery', 'controller' => 'cms', 'action' => 'showlist')));
                }
            }
        } else {
            $messager = new Rastor_Controller_Cms_Messager();
            $messager->setAction('not_found');
            $this->_redirect(Rastor_Url::get('admin', array('module' => 'videogallery', 'controller' => 'cms', 'action' => 'showlist')));
        }
    }

}
