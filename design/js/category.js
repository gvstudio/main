$(function(){
	$('.category ul li').find('a').live('click',function(){ //Ќаходим все теги <a> внутри указаной конструкции и прив€зываем событие клика на них
		var $this = $(this);
		...
		if(sub.size() > 0){ // We have next level
			...
			$this.toggleClass('active', false); // remove "active" class
			} else {
				// ul is invisible
				// close all links of the same level
				$this.parent().siblings().children('ul').slideUp('fast');
				$('.category ul li a').toggleClass('active',false);
				//show it
				sub.slideDown('fast')
